...
inline void*  iftGetElementInVectorAt(const iftGenericVector* vector, size_t index){
    if(IS_INDEX_OUT_BOUNDS(vector,index)){
        iftError("Invalid position","iftGetElementInVectorAt");
        return NULL;
    }
...
inline void* iftGetFrontElementInVector(const iftGenericVector* vector){
    void* offset = (char*)vector->data ;
    return offset;
}

inline void* iftGetBackElementInVector(const iftGenericVector* vector){
    void* offset = (char*)vector->data + (vector->elementSize*vector->size); ;
    return offset;
}

inline void* iftGetDataInVector(const iftGenericVector* vector){
    return vector->data;
}
...
#define VECTOR_INSERT_AT(type, vector, element, index) \
        iftShiftVectorToRightAt(vector,index); \
        ((type*)vector->data)[vector->size] = element; \
        vector->size++;

...
inline void  iftSetCursorPositionInArgumentList(iftArgumentList* list, int index,iftArgumentListNode* node){
    list->currentNodeIndex = index;
    list->currentNode = node;
}

inline void iftAppendElementInArgumentList(iftArgumentList *argumentList, void *element, size_t dataSize){
...
inline void iftPushBackElementInArgumentList(iftArgumentList *argumentList, void *element, size_t dataSize){
    iftAppendElementInArgumentList(argumentList,element,dataSize);
}

inline void iftPushBackNodeInArgumentList(iftArgumentList *argumentList, iftArgumentListNode* node){
    if(argumentList->length == 0) {
        argumentList->head = argumentList->tail = node;
    } else {
        argumentList->tail->next = node;
...
inline void iftPrependElementInArgumentList(iftArgumentList *argumentList, void *element, size_t dataSize){
    iftArgumentListNode* node = iftCreateArgumentNode(element,dataSize);
...
inline void iftPushFrontElementInArgumentList(iftArgumentList *argumentList, void *element, size_t dataSize){
...
inline void* iftGetElementInArgumentList(iftArgumentList *list, size_t index){
...
inline void* iftGetElementInArgumentListByName(iftArgumentList *list, const char* name){
    iftArgumentListNode *currentNode = list->head;
    for (size_t i = 0; i < list->length; ++i) {
